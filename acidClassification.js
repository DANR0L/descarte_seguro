...
function classifyOrganicAcidMixture(compoundNames, hazardData) {
  const allOrganic = compoundNames.every((name) => isOrganicAcid(name));
  const anyButyricValeric = compoundNames.some((name) => {
    const normalized = name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    return normalized.includes('acido butirico') || normalized.includes('acido valerico') || normalized.includes('acido butanoico') || normalized.includes('acido pentanoico');
  });
  ...
  if (allOrganic && corrosive) {
    return UN_CODES.CORROSIVE_LIQUID_ACID_ORGANIC_N_O_S;
  }
  if (anyButyricValeric && isLiquid && corrosive) {
    return UN_CODES.CORROSIVE_LIQUID_ACID_ORGANIC_N_O_S;
  }
