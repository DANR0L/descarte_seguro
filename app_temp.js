...
async function searchPubChem(query) {
        try {
            ...
            if (ghsRes.ok) {
                ...
                try {
                    const sections = ghsData.Record.Section[0].Section[0].Section;
                    for (const section of sections) {
                        if (section.TOCHeading && section.TOCHeading.includes('GHS Classification')) {
                            ...
                            // O break foi removido para varrer TODAS as fontes (ECHA, OSHA, Japan, etc)
                        }
                    }
            ...
            let inferredRiskClass = "A definir";
            let inferredUN = "A definir";
            if (pictograms.has("ghs02_inflamavel")) {
                inferredRiskClass = "3 (Inflamável)";
                inferredUN = "1993"; // Líquido inflamável, N.E.
            } else if (pictograms.has("ghs05_corrosivo")) {
                inferredRiskClass = "8 (Corrosivo)";
                inferredUN = "1760"; // Líquido corrosivo, N.E.
            } else if (pictograms.has("ghs06_toxico") || pictograms.has("ghs08_saude")) {
                inferredRiskClass = "6.1 (Tóxico)";
                inferredUN = "2810"; // Líquido tóxico, N.E.
            } else if (pictograms.has("ghs04_gas")) {
                inferredRiskClass = "2 (Gás)";
                inferredUN = "1956"; // Gás comprimido, N.E.
            } else if (pictograms.has("ghs09_meioambiente")) {
                inferredRiskClass = "9 (Risco Ambiental)";
                inferredUN = "3082"; // Substância de risco ambiental, N.E.
            }
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        if (filtered.length > 0) {
            ...
            searchTimeout = setTimeout(async () => {
                try {
                    const autoRes = await fetch(`https://pubchem.ncbi.nlm.nih.gov/rest/autocomplete/compound/${queryNorm}/json?limit=6`);
                    if (autoRes.ok) {
                        ...
                        if (terms && terms.length > 0) {
                            ...
                            terms.forEach(term => {
                                ...
                                div.addEventListener('click', async (event) => {
                                    ...
                                    const onlineResult = await searchPubChem(term);
                                    if (onlineResult) {
                                        selectProduct(onlineResult);
                                    } else {
                ...
                // Fallback: se o autocomplete falhar, tenta buscar exatamente
                searchResults.innerHTML = '<div class="result-item" style="color: #666;"><i class="fas fa-spinner fa-spin"></i> Pesquisando no PubChem...</div>';
                const onlineResult = await searchPubChem(query);
                searchResults.innerHTML = '';
                if (onlineResult) {
    ...
    function addResiduo(produto) {
        ...
        const newItem = {
            ...
            produto: { ...produto, Common_Name_PT: produto.Common_Name }, // Default to English initially
            percentage: remaining
        };
        currentMixture.push(newItem);
        updateMixtureDisplay();

        // Tenta traduzir automaticamente para Português usando API gratuita
    ...
    function calculateUnifiedLabel() {
        ...
        if (isPureForONU) {
            unifiedOnu = pureComponent.produto.UN_Number || "N/A";
            unifiedClass = pureComponent.produto.Risk_Class || "N/A";
        } else {
            let hasFlammable = false;
            let hasCorrosive = false;
            let hasToxic = false;
            currentMixture.forEach(m => {
                let pList = JSON.stringify(m.produto.Pictograms_List || []).toLowerCase();
                let hList = JSON.stringify(m.produto.H_Phrases || []).toLowerCase();
                if (pList.includes('flame') || hList.includes('h22')) hasFlammable = true;
                if (pList.includes('corrosion') || hList.includes('h314')) hasCorrosive = true;
                if (pList.includes('skull') || hList.includes('h30') || hList.includes('h311') || hList.includes('h331')) hasToxic = true;
            });
            ...
            if (hasFlammable) {
                ...
                unifiedClass = "Classe 3";
            } else if (hasCorrosive) {
                unifiedOnu = `ONU 3264 LÍQUIDO CORROSIVO, ÁCIDO, INORGÂNICO, N.E. (${names})`;
                unifiedClass = "Classe 8";
            } else if (hasToxic) {
                unifiedOnu = `ONU 2810 LÍQUIDO TÓXICO, ORGÂNICO, N.E. (${names})`;
                unifiedClass = "Classe 6.1";
            } else {
                unifiedOnu = "ONU Mistura N.E.";
                unifiedClass = "Variada";
            }
        }
    ...
    document.getElementById('printBtn').addEventListener('click', () => {
        ...
        <div class="form-container size-${configVol}">
            ...
            <div class="print-warning-banner">${unifiedAdv}</div>
